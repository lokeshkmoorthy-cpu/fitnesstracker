  ...
  return (
    <div className="bg-white rounded-3xl p-8 shadow-premium border border-slate-50">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <h2 className="text-xl font-bold tracking-tight text-slate-900 px-2">My Exercise</h2>
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            ...
            <input 
              ...
              className="pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-semibold placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-600/10 focus:border-purple-600 transition-all w-full md:w-64"
            />
          </div>
            ...
            <AnimatePresence mode="popLayout">
              {workouts.slice().reverse().map((workout, idx) => (
                <motion.tr 
                  ...
                  className="group bg-white hover:bg-slate-50/80 transition-all"
                >
                  <td className="px-6 py-5 rounded-l-2xl border-y border-l border-transparent group-hover:border-slate-100">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-sm font-bold text-slate-900 tracking-tight">{workout.exercises}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5 border-y border-transparent group-hover:border-slate-100">
                    <div className="flex flex-col">
                      <span className="text-sm font-semibold text-slate-600 leading-none mb-1">{workout.musclegroup}</span>
                  ...
                  <td className="px-6 py-5 text-center border-y border-transparent group-hover:border-slate-100">
                    <span className="text-sm font-bold text-slate-700">{workout.setsreps}</span>
                  </td>
                  <td className="px-6 py-5 border-y border-transparent group-hover:border-slate-100">
                    <span className="text-sm font-medium text-slate-500">{new Date(workout.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                  </td>
                  <td className="px-6 py-5 rounded-r-2xl border-y border-r border-transparent group-hover:border-slate-100 text-right">
                    <div className="flex items-center justify-end gap-2">
                       {idx % 3 === 0 ? (
                        <div className="px-3 py-1.5 bg-green-50 text-green-600 rounded-lg flex items-center gap-1.5">
                          <CheckCircle2 className="w-3.5 h-3.5" />
                          <span className="text-[11px] font-bold">Completed</span>
                        </div>
                       ) : (
                        <div className="px-3 py-1.5 bg-amber-50 text-amber-600 rounded-lg flex items-center gap-1.5">
