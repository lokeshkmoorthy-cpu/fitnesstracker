...
async function fetchJson<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  const response = await fetch(`${API}${url}`, { ...init, headers }); // ✅ FIXED HERE
...
export const fitnessApi = {
  ...
  getGoals: (user: string) =>
    fetchJson<GoalsRecord[]>(`/api/goals?user=${encodeURIComponent(user)}`),
  ...
  getMotivationQuotes: () =>
    fetchJson<MotivationQuote[]>("/api/motivation-quotes"),

  getBotCommands: () =>
    fetchJson<import("@/src/types/fitness").BotCommand[]>("/api/bot-commands"),
