const exportDashboardPdf = async ({
  ...
  const drawHeaderBand = () => {
    ...
    doc.text(`Generated on ${(/* @__PURE__ */ new Date()).toLocaleString()}`, marginX + 18, y + 54);
    doc.text("Workout + activity + goals snapshot", marginX + 18, y + 70);
    y += 106;
  };
  const drawCard = (x, cardY, width, height, title, value) => {
  ...
  drawCard(marginX + filterWidth + filterGap, y, filterWidth, filterCardHeight, "Search", filters.searchs.trim() || "None");
  ...
  if (!workouts.length) {
    ...
    workouts.forEach((workout, index) => {
      ...
      const wrappedColumns = values.map(
        (value, valueIndex) => doc.splitTextToSize(value, colWidths[valueIndex] - 12)
      );
